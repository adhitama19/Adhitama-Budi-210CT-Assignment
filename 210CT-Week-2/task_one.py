...
def fact_number(number):                  # a new function called fact_number is defined with number as the parameter
    ...
    for i in range (1, number-1):           # a for loop that will go through range 1 to number parameter with less 1 value
    ...
    if number in perfect_square:            # conditional statement to check if the input number in perfect_square list
        return sqrt(number)                 # statements that will return the square root of the input number
        ...
        final_Ans = sqrt(final_val)         # this variable will be the square root of final_val
        
        return "The perfect square of the number less than the input is: %d" % int(sqrt(final_Ans))      #it will then return the sqrt of final_Ans as the answer


print (fact_number(35))
